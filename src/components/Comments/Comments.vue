  ...
  <div class="shadow" align="left">
    ...
    <div>
      ...
      <a style="float: right;font-size: 12px;cursor: pointer;color: dodgerblue;margin-right: 10px" @click="showAnswer"
         v-if="comment.commentAnswers.length != 0">Expand response({{comment.commentAnswers.length}})</a>
      <a style="float: right;font-size: 12px;cursor: pointer;color: dodgerblue;margin-right: 20px"
         @click="answer(comment.commentId,comment.userNickname)">Response</a>
    </div>
    <br>
    <div class="comment" style="margin-left: 40px;background-color: #f2f2f2" v-if="isShowAnswer">
      <div v-for="(commentAnswer,index) in comment.commentAnswers" :key="index">
        ...
        <a style="float: right;font-size: 12px;cursor: pointer;color: #909399;margin-right: 20px"
           @click="answer(comment.commentId,commentAnswer.userNickname)">Response</a>
  ...
  export default {
    ...
    methods: {
      ...
      answer(parentId, commentTo) {
        if (!this.user.userId) {
          this.$message.warning("Login to reply")
          return
        }
        this.$prompt('Reply', {
